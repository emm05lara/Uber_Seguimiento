...
EXCEL_PATH_DEFAULT = "prueba.xlsx"
SHEET_NAME_DEFAULT = "GENERAL"  # antes: SHEET_NAME fijo; ahora lo usamos como default
...
# ✅ NUEVO: leer nombres de hojas sin afectar tu lógica
def get_excel_sheets(excel_path: str):
    try:
...
with st.sidebar:
    ...
    # ✅ NUEVO: selector de hoja (sin tocar tu lógica de procesamiento)
    sheets = get_excel_sheets(excel_path)
    if sheets:
        sheet_name = st.selectbox(
            "Hoja de Excel a visualizar",
            sheets,
            index=sheets.index(SHEET_NAME_DEFAULT) if SHEET_NAME_DEFAULT in sheets else 0
        )
    else:
        st.error("No pude leer el Excel para obtener las hojas. Revisa la ruta/archivo.")
        st.stop()
...
if vista == "🚗 Flota":
    ...
    # KPIs calculados SOBRE EL FILTRADO
    verificados = int(df_f["FLAG_VERIFICADO"].sum()) if "FLAG_VERIFICADO" in df_f.columns else 0
    tenencia_ok = int(df_f["FLAG_TENENCIA_OK"].sum()) if "FLAG_TENENCIA_OK" in df_f.columns else 0
    con_adeudo = int(df_f["FLAG_ADEUDO"].sum()) if "FLAG_ADEUDO" in df_f.columns else 0
    ...
    poliza_exp = count_expiring(df_f["VIGENCIA_DT"], warn_days) if "VIGENCIA_DT" in df_f.columns else 0
    tcirc_exp = count_expiring(df_f["TCIRC_DT"], warn_days) if "TCIRC_DT" in df_f.columns else 0
